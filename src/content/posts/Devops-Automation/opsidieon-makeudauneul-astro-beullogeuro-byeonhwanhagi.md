...
Obsidian에서 작성한 기술 블로그 마크다운 파일을 Astro Content Collections에 맞게 변환해야 한다. Obsidian은 `위키 링크`, `![](../../assets/f652efbe-이미지.png)` 같은 고유 문법을 사용하고, Attachments 폴더에 이미지를 별도 관리한다. 이런 차이를 자동으로 처리하는 마이그레이션 스크립트를 작성했다.
...
원본 제목과 슬러그 두 가지 키로 등록해, `[PostGIS RPC로 구역 저장과 공간 조회](/post/postgis-rpcro-guyeok-jeojanggwa-gonggan-johoe)`와 `postgis-rpc-ro-...` 모두 같은 URL로 해석된다.

# 이미지 처리

Obsidian의 이미지 참조(`![](../../assets/f652efbe-image.png)`, `![alt](../../assets/f652efbe-image.png)`)를 추출하고, Attachments 폴더에서 이미지를 복사한 뒤 경로를 변환한다.
...
function normalizeImages(content, renameMap, assetPrefix) {
  // ![](../../assets/f652efbe-img.png) → ![](../../assets/f652efbe-hash-name.png)
  const wikiFixed = content.replace(/!\[\[([^[\]]+)\]\]/g, (_m, p1) => {
    const raw = p1.trim();
    if (raw.startsWith('http')) return `![](../../assets/${raw})`;
    const mapped = renameMap.get(raw) ?? raw;
    return `![](../../assets/${assetPrefix}${mapped})`;
  });

  // ![alt](../../assets/f652efbe-img.png) → ![alt](../../assets/f652efbe-hash-name.png)
  return wikiFixed.replace(/!\[([^\]]*)]\(([^)]+)\)/g, (_m, alt, url) => {
    const trimmed = url.trim();
    if (trimmed.startsWith('http')) return `![${alt}](../../assets/${trimmed})`;
    const mapped = renameMap.get(path.basename(trimmed)) ?? path.basename(trimmed);
    return `![${alt}](../../assets/${assetPrefix}${mapped})`;
  });
}
```
