    ...
    if (
      ...
      if (['create', 'update', 'updateMany'].includes(params.action)) {
        ...
        if (
          ...
          params.args.data.phoneNumberEncrypted = true;
        }
      }
...
export class SecureDataService {
  ...
  private scheduleEncryptionTask() {
    const timeout = setTimeout(() => this.encrypteAll(), 3000000);
...
이 작업을 할 때에는 데이터베이스의 백업이 필수적이다. 중간에 무언가 코드가 잘못 꼬인다면 암호화된 이메일을 다시 암호화한다던지, 공백을 복호화하려고 해, 오류가 난다든지등의 문제가 계속해서 나타난다.
...
- [파일 암호화 파이프라인 구현](/post/pail-amhohwa-paipeurain-guhyeon)
- [NestJS GraphQL에서 역할 기반 접근 제어 구현하기](/post/nestjs-graphqleseo-yeokal-giban-jeopgeun-jeeo-guhyeonhagi)
