...
[[SVG 아이콘을 React 컴포넌트 라이브러리로 만들어 자동 배포하기|이전 글]]에서 구매한 SVG 아이콘을 React 컴포넌트 라이브러리로 만들어 자동 배포하는 시스템을 구축했다. 웹 프로젝트에서는 이걸로 충분했는데, 문제가 하나 더 있었다. 우리 팀은 React Native 앱도 운영하고 있었다.
