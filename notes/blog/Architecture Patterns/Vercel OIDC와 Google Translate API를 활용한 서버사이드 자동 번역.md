...
[[React Native 앱의 다국어 지원 구현]]에서 i18next 기반 정적 번역 파일과 서버사이드 푸시 알림 번역을 다뤘다. 정적 UI 텍스트는 번역 파일로 해결되지만, 사용자가 생성하는 동적 콘텐츠(장소 이름, 주소, 설명, 공지사항 등)는 미리 번역해둘 수 없다. 관리자가 장소 정보를 수정할 때마다 5개 언어로 수동 번역하는 것은 현실적이지 않았다.
